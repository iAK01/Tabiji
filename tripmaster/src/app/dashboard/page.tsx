...
import {
  ...
  ToggleButtonGroup, ToggleButton, Tabs, Tab, Paper, alpha,
} from '@mui/material';
...
function ActiveTripBanner({ trip, onOpen }: { trip: Trip; onOpen: () => void }) {
    ...
    >
      ...
      <Box sx={{
        ...
        background: trip.coverPhotoUrl
          ? `linear-gradient(to bottom, ${alpha('#000', 0.1)}, ${alpha('#000', 0.6)}), url(${trip.coverPhotoUrl}) center/cover no-repeat`
      ...
      {/* Progress bar */}
      <Box sx={{ height: 5, backgroundColor: alpha('#55702C', 0.15) }}>
      ...
      <Box sx={{
        ...
        backgroundColor: alpha('#55702C', 0.04),
      }}>
                    ...
                    return (
                      <Card key={trip._id} onClick={() => router.push(`/trips/${trip._id}`)}
                        sx={{
                          ...
                          // Active trip gets a green accent in the card list too
