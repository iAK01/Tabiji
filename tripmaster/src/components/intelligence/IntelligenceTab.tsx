...
function AddToItineraryDialog({
  ...
  const stopType = (h: CultureHighlight) => {
  ...
  const defaultDuration = (h: CultureHighlight) => {
      ...
      <DialogContent sx={{ pt: 0 }}>
        ...
        {done ? (
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
            {/* Day picker (only shown if multiple days) */}
            {days.length > 1 && (
              <Box>
                <Typography fontWeight={700} sx={{ fontSize: '0.88rem', mb: 0.75, textTransform: 'uppercase', letterSpacing: 0.5, color: 'text.secondary' }}>
                  Which day?
                </Typography>
                <List disablePadding>
                  {days.map(day => (
